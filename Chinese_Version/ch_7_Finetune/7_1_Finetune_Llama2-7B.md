...
为了帮助您更好地理解微调过程，在本教程中，我们提供了一个实用指南，利用 BigDL-LLM 将大语言模型对特定的下游任务进行微调。 这里使用 [Llama-2-7b-hf](https://huggingface.co/meta-llama/Llama-2-7b-hf) 作为示例来完成文本生成任务。
...
按照[Readme](./README.md#70-environment-setup)中的步骤设置环境后，您可以使用以下命令在终端中安装BigDL-LLM：
...
安装后，让我们转到本教程的 **Python 脚本**。 首先，您需要先导入`intel_extension_for_pytorch`以启用 BigDL-LLM 优化：
...
本教程选择流行的开源LLM [meta-llama/Llama-2-7b-hf](https://huggingface.co/meta-llama/Llama-2-7b-hf)来说明模型微调的过程。
...
接下来，我们可以从预处理后的模型中创建一个PEFTModel并配置它的参数，如下所示：
...
config = LoraConfig(r=8, # 定义训练参数的多少
                    lora_alpha=32, # 调整权重矩阵的大小
                    target_modules=["q_proj", "k_proj", "v_proj"], 
                    lora_dropout=0.05, # 避免过拟合
                    bias="none", 
                    task_type="CAUSAL_LM")
...
> **注意**
>
> 我们从`bigdl.llm.transformers.qlora`导入与BigDL-LLM兼容的PEFTModel，替代原先使用bitandbytes库和cuda的`from peft importprepare_model_for_kbit_training, get_peft_model`用法。其使用方法和用`peft`库进行QLoRA微调方法相同。

> **注意**
>
> 对于 Llama 2 (7B) 模型，`target_modules` 设置为 `["q_proj", "k_proj", "v_proj"]`。
> 您可以在 [Hugging Face 文档](https://github.com/huggingface/peft/blob/39ef2546d5d9b8f5f8a7016ec10657887a867041/src/peft/utils/other.py#L220) 上找到对于不同模型的target_modules列表。

### 7.1.2.3 加载数据集

我们加载通用数据集 [englishquotes](https://huggingface.co/datasets/Abirate/english_quotes) 来根据英语名言来微调我们的模型。
...
> **注意**
>
> 如果您已经从 [Abirate/english_quotes](https://huggingface.co/datasets/Abirate/english_quotes/blob/main/quotes.jsonl) 下载了 `.jsonl` 文件，您可以使用 `data = load_dataset( "json", data_files= "path/to/your/.jsonl/file")` 指定本地路径，以替代从huggingface repo id的加载方法 `data = load_dataset("Abirate/english_quotes")`。

### 7.1.2.4 加载Tokenizer

您可以使用 [Huggingface Transformers](https://huggingface.co/docs/transformers/index) API来加载LLM 推理需要的分词器，它可以与 BigDL-LLM 加载的模型无缝配合使用。对于Llama 2，对应的tokenizer类为`LlamaTokenizer`。
...
现在您可以通过使用HuggingFace系统中的现有工具设置`trainer`来开始训练过程。 这里我们将`warmup_steps`设置为 20 以加速训练过程。
```python
import transformers
...
最终的LoRA模型和配置会保存到`${output_dir}/checkpoint-{max_steps}`。
...
> **注意**
>
> 请确保您的`accelerate`库版本是 0.23.0，以便在CPU上启动模型合并。
...
> **注意**
>
> 我们用`bigdl.llm.transformers.qlora`替代`from peft import PeftModel`来导入BigDL-LLM兼容的PEFT模型。

> **注意**
> 
> 适配器路径是您保存微调模型的本地路径，在我们的例子中是“./outputs/checkpoint-200”。
>
...
我们仅替换`model_path`参数来使用预训练模型重复该过程，以验证微调过程后的改进。
...
我们可以看到微调模型的推理结果与新增数据集中有相同的词汇以及近似的文本风格。基于BigDL-LLM的优化，我们可以在仅仅几分钟的训练之内达到这个效果。
